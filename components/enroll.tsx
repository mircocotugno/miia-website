import type { EnrollProps } from '@cms/components'
...
export function Enroll({ blok }: EnrollComponent) {
  const options = blok.courses.map((course) => {
    const key = `${course.hours} - ${course.location.title} - ${course.hours.includes('20:00/23:00') ? 'serale' : 'sabato'}`
          ...
          <div className='sticky md:top-20 col-span-full md:col-span-4 flex flex-col align-start justify-start -mt-10 md:-mt-20 lg:-mt-48 px-2 py-3 order-1 md:order-last max-h-fit bg-background shadow-lg rounded-3xl border-neutral-100/25 border-2'>
            ...
            {!!blok.courses.length && (
              <Accordion selectionMode='multiple'>
                {blok.courses.map((course, index) => (
                  <AccordionItem
                    title={
                      <h4 className='font-bold tex-xl'>
            ...
            {blok.form && (
              <StoryblokComponent blok={blok.form} courses={options} />
            )}
          </div>
        </div>
      )}
